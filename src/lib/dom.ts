...
const lib = (() => {
  ...
  const _extendHtmlElementPrototype = () => {
     ...
     * @param condition
     */
    HTMLElement.prototype.getParentByCondition = function(
      condition: (parent: HTMLElement) => boolean
  ...
  const _appendChildren = (
    ...
    if (Array.isArray(childs)) {
      childs.map(
        (childElement: HTMLElement | SVGSVGElement | Text, index: number) => {
          _appendChildren(targetElement, childElement);
        }
      );
    } else {
      if (
        childs instanceof HTMLElement ||
        childs instanceof SVGSVGElement ||
        childs instanceof Text
      ) {
