...
const buildMetaInfo = (props:Array<string|{
    name:string,
    type?:string|string[]
...
export const drawEngine2dMeta = {
    ...
    hexagon: buildMetaInfo(['position', 'radius', 'strokeStyle', 'fillStyle', 'angle']),
    image: buildMetaInfo(['canvas', 'sourceRect', 'destinationRect', 'angle', 'pivot']),
    lines: buildMetaInfo(['startPosition', 'destinations', 'strokeStyle', 'fillStyle', 'lineWidth']),
    moveTo: buildMetaInfo(['position']),
    quadraticCurveTo: buildMetaInfo(['controlPoint', 'position']),
    rectangle: buildMetaInfo(['rect', 'strokeStyle', 'fillStyle', 'angle', 'pivot']),
    removeDrawCondition: {},
    resetClip: {},
