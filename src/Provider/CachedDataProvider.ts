...
export default class CachedDataProvider {
        ...
        return new Promise((resolve, reject) => {
            ...
            if(pendingRequestInfo.state === 'loading'){
                ...
                fetch(fetchOptions.url)
                    .then((response) => {
                        response[fetchOptions.evalJson ? 'json' : 'text']().then(r=>{
    ...
    private _buildCacheKey(fetchOptions:ICachedDataProviderFetchOptions):string {
        return 'C_' + btoa(encodeURI(JSON.stringify(fetchOptions.method + fetchOptions.url + JSON.stringify(fetchOptions.data))));
    }
    private _getPendingRequest(cacheKey:string): IPendingRequest & { index:number } {
        ...
        this._pendingRequests.push({
            cacheKey,
            callbacks: {reject: [], resolve: [(response:any)=>{
                this._cache[cacheKey] = response;
            }]},
            state: 'initiated'
        });
        return {...this._pendingRequests[this._pendingRequests.length - 1], index: this._pendingRequests.length - 1};
    }
    private _handleResponse(cacheKey:string, response:Response, callbackType:string = 'resolve'):void {
        ...
        this._pendingRequests[pendingRequest.index].state = callbackType === 'resolve' ? 'resolved' : 'rejected';


        (pendingRequest.callbacks as any)[callbackType].map((callback:CallableFunction) => {
            callback(response)
        });
