...
export default class Vector2D implements IVector2D {
  ...
  public get positionSaveX(): number {
    return this.positionType === EPositionType.absolute
      ? this.x
      : this.cameraSaveX;
  }
    ...
    return {
      ...
      getNomalizedMoveVector: (speed: IVector2D = { x: 1, y: 1 }) => {
        ...
        if (otherVector.x === this.x) {
          return {
            x: 0,
            y: (otherVector.y > this.y ? 1 : -1) * Math.min(speed.y, distance)
          };
        }
        if (otherVector.y === this.y) {
          return {
            x: (otherVector.x > this.x ? 1 : -1) * Math.min(speed.x, distance),
            y: 0
          };
        }

        const movementVector = {
          x: otherVector.x - this.x,
          y: otherVector.y - this.y
        };
        const vectorLength = Math.sqrt(
          Math.pow(movementVector.x, 2) + Math.pow(movementVector.y, 2)
        );
        ...
        return {
          x:
            normalizedVector.x > 0
              ? Math.min(normalizedVector.x, otherVector.x - this.x)
              : Math.min(normalizedVector.x, this.x - otherVector.x),
          y:
            normalizedVector.y > 0
              ? Math.min(normalizedVector.y, otherVector.y - this.y)
              : Math.min(normalizedVector.y, this.y - otherVector.y)
        };
      },
      subtract: () => {
  ...
  public rotate(angle: number, anchor: IVector2D): Vector2D {
    ...
    const newY =
      cos * (this.y - anchor.y) - sin * (this.x - anchor.x) + anchor.y;
