...
export default class CanvasEngineAnalyzer extends Canvas2dDrawEngine {
  ...
  constructor(
    ...
    listener?: (reason: EAnalyzeReason, props: {}) => void
  ) {
    super();
  ...
  public analyze(
    ...
    switch (reason) {
      case EAnalyzeReason.METHOD_START:
        additionalInfos.depth = this._methodStack.filter(
          m => m.methodName !== "start"
        ).length;
        ...
        if (props.methodName !== "start") {
          _ignore = !(
            (this._config.analyzingDepth < 0 ||
              additionalInfos.depth <= this._config.analyzingDepth) &&
  ...
  private _doSelfCheck(): void {
    ...
    if (
      this._config.checkForStartTrigger &&
      this._rootPosition > 0 &&
      this._analyzerStartedAt === -1
    ) {
    ...
    if (this._config.checkLoopEnd && this._methodStack.length === 0) {
      ...
      this._analyzerFinishedCheckTimeout = setTimeout(() => {
        if (this._listener) {
          this._listener(EAnalyzeReason.METHOD_END, {
            methodName: "start",
            endTime: _now,
            duration: _now - this._analyzerStartedAt
          });
        }
      }, 100);
    }
