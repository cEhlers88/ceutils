...
export default class {
    ...
    constructor() {
        ...
        return new Proxy(this,{
            ...
            set(target: any, p: string | number | symbol, value: any, receiver: any): boolean {
                if (typeof target[p] !== 'undefined') return target[p]=value;
    ...
    public getData(key:string):any{
        const entry:{key:string,value:any}|undefined=this.data.find((entry:{key:string,value:any})=>entry.key===key);
        return (entry?entry.value:undefined);
    }
    public getDataIndex(key:string):number|undefined{
        let result:number|undefined=undefined;
        for(let index:number=0;index<this.data.length;index++){
            if(this.data[index].key===key){result=index;break;}
        }
        return result;
    }
    public setData(key:string, value:any){
        const index:number|undefined=this.getDataIndex(key);
        if(index!==undefined){this.data[index].value=value;}else{
