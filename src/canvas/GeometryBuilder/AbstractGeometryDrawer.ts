...
export default abstract class AbstractGeometryDrawer {
    protected _context:CanvasRenderingContext2D|WebGL2RenderingContext|null = null;
    protected _helpers: {
        rotate: (points: IVector2D[]|IVector3D[], rotationPoint: IVector2D|IVector3D, angle:number|IVector2D|IVector3D) => IVector2D[]|IVector3D[];
    } = {
        rotate: (points: IVector2D[]|IVector3D[], rotationPoint: IVector2D|IVector3D, angle:number|IVector2D|IVector3D) => {
            return points;
        }
    }
    public create(geometryArgs:IGeometryArgs):IGeometryDrawerCreation{
        const _buildReturnObject = (currentArgs:IGeometryArgs, currentStore:IGeometryStore)=> {
            return {
                draw: () => {
                    this._handleDraw(currentStore);
                },
                update: (updateArgs:IGeometryArgs) => {
                    return _buildReturnObject(updateArgs, this._update(currentStore, updateArgs));
                }
            };
        };
    ...
    public getParallelEdges(edges:IVector2D[]|IVector3D[], edge:IVector2D|IVector3D|null=null):IVector2D[]|IVector3D[] {
        const result:any = [];
        const _pushResult = (sEdge:IVector2D|IVector3D) => {
            if(!result.some((currentEdge:any) => currentEdge.x === sEdge.x && currentEdge.y === sEdge.y)){
                result.push(edge);
            }
        };
        ...
        edges.map((currentEdge) => {
            if(edge===null){
                ...
                if (Math.round(currentEdge.x) === Math.round(edge.x) || Math.round(currentEdge.y) === Math.round(edge.y)) {
    ...
    public setContext(context:CanvasRenderingContext2D|WebGL2RenderingContext):AbstractGeometryDrawer {
        ...
        if(this._context! instanceof WebGL2RenderingContext){
            ...
            this._helpers.rotate = (points: IVector2D[], rotationPoint: IVector2D, angle:number|IVector2D) => {
                ...
                return points.map((point) => {
                    ...
                    return {
                        x: _x * Math.cos(_angle.x) - _y * Math.sin(_angle.x) + rotationPoint.x,
                        y: _x * Math.sin(_angle.y) + _y * Math.cos(_angle.y) + rotationPoint.y
                    };
                });
            }
    ...
    protected _update(store: IGeometryStore, updateArgs: IGeometryArgs): IGeometryStore {
        if (updateArgs.angles.length > 0) {
            ...
            _angles.map((angle, index) => {
                ...
                if(updateArgs.rotateType === ERotateType.center){
                    const movedCenter = this._helpers.rotate([{x:(store.width/2),y:(store.height/2)}], {
                    ...
                    store.edges = this._helpers.rotate(store.edges, {
                        x: _pivot.x * store.width,
                        y: _pivot.y * store.height
                    }, angle);
                }
    ...
    private _handleDraw:(drawArgs:IGeometryStore)=>void = (store:IGeometryStore)=> {
        return;
    };
