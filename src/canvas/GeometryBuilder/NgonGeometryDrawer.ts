...
export default class NgonGeometryDrawer extends AbstractGeometryDrawer {
  protected _create(creationArgs: INgonGeometryArgs): IGeometryStore {
    ...
    const angleRad =
      (((creationArgs.angles![0] as number) || 0) * Math.PI) / 180;
    ...
    for (let i = 0; i <= creationArgs.sides; i++) {
      ...
      result.edges.push({
        x:
          _radius * Math.cos(rotAngle + angleRad) +
          pivotCenterDiff.x * result.width,
        y:
          _radius * Math.sin(rotAngle + angleRad) +
          pivotCenterDiff.y * result.height,
        z:
          "z" in creationArgs.position
            ? (creationArgs.position as IVector3D).z
            : 0
      });
    }
