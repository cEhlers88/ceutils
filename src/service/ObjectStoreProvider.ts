...
export default class ObjectStoreProvider {
  ...
  public addObject(
    ...
    let _storedObject: IStoredObject | undefined = window.__ObjectStore.find(
      storedObject => storedObject.name === name
    );
    while (_storedObject) {
      const _split = this.splitByLastNumbers(_storedObject.name);
      name = _split.name + (_split.number + 1);
      _storedObject = window.__ObjectStore.find(
        storedObject => storedObject.name === name
      );
    }
  ...
  public getObject(name: string): any {
    const _storedObject: IStoredObject | undefined = window.__ObjectStore.find(
      storedObject => storedObject.name === name
    );
    if (_storedObject) {
  ...
  public getObjectsByGroup(group: string = "default"): any[] {
    return window.__ObjectStore
      .filter(storedObject => storedObject.group === group)
      .map(storedObject => storedObject.instance);
  }
  public getStore(): any[] {
    return window.__ObjectStore;
  }
  public getStoreInfo(objectName: string): any {
    const _storeIndex = window.__ObjectStore.findIndex(
      storedObject => storedObject.name === objectName
    );
    return {
  ...
  public removeObject(name: string): ObjectStoreProvider {
    window.__ObjectStore = window.__ObjectStore.filter(
      storedObject => storedObject.name !== name
    );
    return this;
  }
