...
export default class ComponentStarter {
    ...
    public execute(
        ...
        const self = this;
        this.getScriptsToEvaluate(domEntryPoint).map(self.renderComponent.bind(self));
        return this;
    }
    ...
    public getService(serviceName:string):IService|undefined {
        return this.servicePool.find((service:IService)=>(serviceName === service.getName()))
    }
    ...
    public getScriptsToEvaluate(
        ...
        // @ts-ignore
        domEntryPoint.querySelectorAll("script[type='"+this.config.typeNeedle+"']").forEach((element:HTMLScriptElement)=>{
    ...
    public registerComponent(
        ...
        if(component instanceof AbstractService){
            ...
            try {
                this.doRegisterComponent(new component({}), component, componentName, namespace);
            }catch(e){
    ...
    private doRegisterComponent(
        ...
        if(componentInstance instanceof AbstractService){
            throw Error('Services needs to be initialized before add to ComponentStarter');
        }
        if(componentInstance instanceof ReactComponent){
            ...
            type = ((componentInstance instanceof React.Component) ? eComponentType.reactComponent : eComponentType.function);
        }
     ...
     * @param element
     */
    private getScriptElementInfos(element:HTMLScriptElement):IScriptElementInformations{
        let props={};
        try{
            ...
            this.debug('Error while parsing componentsettings',{e,element},'warn');
        }
        return {
            componentName:(element.hasAttribute('component')?element.getAttribute('component') || '':''),
            componentNamespace:(element.hasAttribute('namespace')?element.getAttribute('namespace') || ComponentStarter.DEFAULT_NAMESPACE:ComponentStarter.DEFAULT_NAMESPACE),
            componentProperties:props,
            element,
            isEnabled:!(element.hasAttribute('disabled') && element.getAttribute('disabled')==='true'),
            renderElement:(element.hasAttribute('renderElement')?document.getElementById(element.getAttribute('renderElement') || ''):null)
        };
    }
     ...
     * @param element
     */
    private isScriptElementEvaluatable(element:HTMLScriptElement):IScriptElementValidationResult {
        const elementInfos:IScriptElementInformations = this.getScriptElementInfos(element);
        ...
        if(elementInfos.isEnabled){
            if(!this.getRegisteredComponentEntry(elementInfos.componentName,elementInfos.componentNamespace)){
    ...
    private renderComponent(scriptElement:HTMLScriptElement):void{
        ...
        const componentEntry = this.getRegisteredComponentEntry(scriptinfos.componentName,scriptinfos.componentNamespace);
        ...
        if(componentEntry!.requiredServices.length>0){
            ...
            componentEntry!.requiredServices.map((serviceName:string)=>{
                ...
                if(tmpService){
                    ...
                    self.debug(`Missing required service "${serviceName}" to render component "${componentEntry!.namespace}\\${componentEntry!.name}"`,{scriptinfos,componentEntry},'warn');
                }
            });
        }

        if(eComponentType.reactComponent === componentEntry!.type || eComponentType.managedReactComponent === componentEntry!.type) {
            if(renderInScriptElement){
                scriptinfos.renderElement = document.createElement("div");
            }
            ReactDOM.render(
                React.createElement(componentEntry!.component,scriptinfos.componentProperties),
                ...
                try{
                    ...
                    if(scriptinfos.renderElement!.childNodes!.length>0){
                        while(scriptinfos.renderElement!.firstChild){
                            domEntryParent.insertBefore(scriptinfos.renderElement!.firstChild!,firstSibling);
                        }
                    }
                }catch(e){
                    this.debug('Error while rendering component',{
                        componentEntry,
                        e,
                        scriptinfos
                    },'error');
            ...
            if(renderInScriptElement){
                scriptinfos.element.parentNode!.replaceChild(componentEntry!.component,scriptinfos.element);
            }else{
                scriptinfos.renderElement!.parentNode!.replaceChild(componentEntry!.component,scriptinfos.renderElement!);
            }
        }else{
            this.debug(`new Service ${componentEntry!.component.name}`);
            const newService = new componentEntry!.component(scriptinfos.componentProperties);
        }
    }
